...
def prompt_player_wants_instructions():
    ...
    print()
    prompt("Press 'y' or 'n': ", ending='')
    response = return_valid_yn_response('Would you like to see \
the instructions?', input().casefold().strip())
...
def instructions():
    ...
    prompt("Press 'y' or 'n': ", ending='')
    response = return_valid_yn_response("Do you need to see the \
instructions again?", input().casefold().strip())
    if response == 'y':
        transition()
        instructions()
...
def one_round(deck, player_hand, dealer_hand, player_name, score_table):
    ...
    player_points, dealer_points = \
        compare_hands_return_both_scores(player_hand, dealer_hand)
    ...
    if stay_or_bust == 'stay':
        ...
        if stay_or_bust == 'stay':
            player_points, dealer_points = \
                compare_hands_return_both_scores(player_hand, dealer_hand)
            winner = return_winner_after_comparing(player_points,
                                                   dealer_points, player_name)
        else:
            winner = player_name
...
def calculate_ace_points(hand, no_ace_total):
    hand_faces = [card[0] for card in hand]
    aces_count = hand_faces.count('ace')
    aces_only_total = aces_count * ACE_MAX
    total_val = no_ace_total + aces_only_total

    while aces_count and total_val > 21:
        aces_only_total -= 10
        total_val -= 10
        aces_count -= 1
...
def display_dealer_hand(dealer_hand):
    prompt(f"{DEALER} has: an unknown card and the \
{dealer_hand[0][0]} of {dealer_hand[0][2]}.")
    print()
    pause()
...
def ends_with_stay(winner, player_points, dealer_points):
    if winner == DEALER:
        ...
        prompt("It's a push! That means you tied, so no winners this round.")
...
def match_is_over(score_table):
    return ROUNDS_TO_WIN in (score_table[0][1], score_table[1][1])
...
def prompt_play_next_round():
    ...
    print()
    prompt("Press 'y' or 'n': ", ending='')
    response = return_valid_yn_response("Would you like to continue \
to the next round?", input().casefold().strip())
